...
After scaffolding, the skill prints a reminder to run `/install-github-app` from inside the new repo. That command installs the Claude GitHub App on the repo and configures the `CLAUDE_CODE_OAUTH_TOKEN` secret needed by the `claude.yml` workflow this scaffold writes. The scaffold's `claude.yml` is intentionally identical to what `/install-github-app` produces, so you can decline the overwrite prompt during the install.
...
スキャフォールド完了時、新リポジトリ内で `/install-github-app` を実行するようリマインダが表示されます。このコマンドは Claude GitHub App をリポジトリにインストールし、生成された `claude.yml` ワークフローが必要とする `CLAUDE_CODE_OAUTH_TOKEN` シークレットを設定します。スキャフォールドが書く `claude.yml` は `/install-github-app` が生成するものと意図的に同一のため、インストール中に上書き確認が出た場合は拒否しても構いません。
